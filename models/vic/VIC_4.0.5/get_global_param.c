...
**********************************************************************/
{
  ...
  ******************************************/
  ...
  if((options.FULL_ENERGY || options.FROZEN_SOIL) && options.Nnode<3) {
    sprintf(ErrStr,"You must define at least 3 soil thermal nodes to run the model in FULL_ENERGY or FROZEN_SOIL modes.  Currently Nnodes is set to  %d.",options.Nnode);
    nrerror(ErrStr);
  }
  if(!options.QUICK_FLUX && options.Nnode<4) {
    sprintf(ErrStr,"You must define at least 4 soil thermal nodes to run the model with the finite difference ground heat flux solution.  Currently Nnodes is set to  %d.",options.Nnode);
    nrerror(ErrStr);
  }
