  ...
  **********************************************************************/
{
  ...
  ***************************/
  ...
  if(param_set.FORCE_FORMAT[file_num] == BINARY){
    ...
    if (feof(infile))
      nrerror("No data for the specified time period in the binary forcing file.  Won't read file. Model stopping...");
  ...
  **************************/
    ...
    for(i=0;i<skip_recs;i++){
      if( fgets(str, MAXSTRING, infile) == NULL ){
        fprintf(stdout,"i = %d, skip_recs = %d\n", i, skip_recs);
        fprintf(stdout,"forceskip = %d dt = %d\n", global_param.dt, forceskip);
	nrerror("No data for the specified time period in the ASCII forcing file.  Model stopping...");
      }
    }
