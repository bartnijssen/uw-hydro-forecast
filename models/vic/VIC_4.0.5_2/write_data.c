...
/**********************************************************************
  ...
  02-22-05      added AWW's modifications to reduce snow output ASA
**********************************************************************/
{
...
/*     fwrite(tmp_fptr,1,sizeof(float),outfiles->snow); */
    ...
    /* --  AWW disabled following to reduce output - put swe[0] into flux file --
  ...
  if(options.PRT_SNOW_BAND && options.BINARY_OUTPUT) {
    ...
    for(band=0;band<options.SNOW_BAND;band++) {
      ...
      fprintf(outfiles->snowband,"\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f",
        ...
        ------- by replacing last with next: */
      fprintf(outfiles->snowband  ,"\t%.1f", out_data->swq[band+1]);
      /* END REPLACEMENT: AWW */
    }
    ...
    /* AWW: following commented out to reduce band output to swe only :
    for ( band = 0; band < options.SNOW_BAND; band++ ) {
    ...
    ----- using replacement: */
    for ( band = 0; band < options.SNOW_BAND; band++ )
      fprintf(outfiles->snowband  ,"\t%.4f", out_data->swq[band+1]);
    /* -------- end replacement: AWW --- */
...
/* ------------ AWW replace the following with -------------
   ...
   ------------------- AWW with next ----------------- */
    fprintf(outfiles->fluxes,"\t%.3f\t%.3f\t%.3f\t%.3f\t%.2f",
            out_data->prec, out_data->evap, out_data->runoff,
            out_data->baseflow, out_data->air_temp);
    for ( j = 0; j < options.Nlayer; j++ )
      fprintf(outfiles->fluxes,"\t%.1f", out_data->moist[j]);
    fprintf(outfiles->fluxes, "\t%.2f", out_data->swq[0] );
    fprintf(outfiles->fluxes,"\t%.4f\t%.4f\n",
                out_data->r_net, out_data->rel_humid);
