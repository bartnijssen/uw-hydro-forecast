...
$PROG = "$TOOLS_DIR/SPI/spi-0.2/spi";
...
$LogFile = "$LogDir/log.SPI_model.pl.$JOB_ID";
...
$cmd = "$TOOLS_DIR/ndxes/calc.vars_1mo_rolling.pl $Cyr $Cmon $Cday $Flist $RetroDir $NearRTDir $RTDir $NDXESDIR $Clim_Syr >& $LogFile.tmp; cat $LogFile.tmp >> $LogFile";
print "$cmd\n";
(system($cmd)==0) or die "$0: ERROR: $cmd failed: $?\n";

# calculated indices
$cmd = "$TOOLS_DIR/ndxes/calc.std_indices.scr $DATE $PROG $NDXESDIR $Clim_Syr $Clim_Eyr >& $LogFile.tmp; cat $LogFile.tmp >>$LogFile";
print "$cmd\n";
(system($cmd)==0) or die "$0: ERROR: $cmd failed: $?\n";
