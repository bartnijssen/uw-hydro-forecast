...
$cmd = "$TOOLS_DIR/calc.per_stats.CLIM.pl.bak $Cyr $Cmon $Cday $Clim_Syr $Clim_Eyr $Flist $RetroDir $NearRTDir $RTDir $ESP $MODEL >& $LogFile.tmp";
print "$cmd\n";
...
$cmd = "$TOOLS_DIR/calc.ens_fcst_stats.20-33.pl.bak $Cyr $Cmon $Cday $Clim_Syr $Clim_Eyr $Ens_Syr $Ens_Eyr $Flist $NearRTDir $RTDir $ESP $MODEL >& $LogFile.tmp";
print "$cmd\n";
(system($cmd)==0) or die "$0: ERROR: $cmd failed: $?\n";
$cmd = "cat $LogFile.tmp >> $LogFile";
print "$cmd\n";
...
## calc stats ESPO ensemble -------------

#$cmd = "$TOOLS_DIR/calc.ens_fcst_stats.SUBSET.20-33.pl $Cyr $Cmon $Cday $Clim_Syr $Clim_Eyr $Flist $NearRTDir $RTDir $ESP $MODEL >& $LogFile.tmp";
#print "$cmd\n";
