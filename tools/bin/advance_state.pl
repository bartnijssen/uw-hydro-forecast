...
if ($stage == 1) {
  ...
  $cmd = "$TOOLS_DIR/clear_curr_forcings.pl $PROJECT $OldStartDate >& " .
    "$LogFile.tmp";
  print "$cmd\n";
...
# ------------------------------------------------------------------------------
if ($stage == 2) {
  $cmd = "$TOOLS_DIR/update_forcings_asc_advance_state.pl $PROJECT " .
...
if ($stage == 3) {
  foreach $model (@models) {
    ...
    if ($ForcingType ne "nc") {
      $cmd = "$TOOLS_DIR/nowcast_model.pl $PROJECT $model model >& " .
        "$LogFile.tmp";
      print "$cmd\n";
      (system($cmd)==0) or die "$0: ERROR: $cmd failed: $?\n";
      $cmd = "cat $LogFile.tmp >> $LogFile";
