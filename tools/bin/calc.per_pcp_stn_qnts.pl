...
### Number of periods is decided by comparing the current day with the start of spinup period 
### Until $Mindays th day of the current month, all the days in the current month + past month is considered to be the 2nd period and the first month is considered to be the 1st period
### Advance state runs on $Min_days th day of the month and moves the start of the current spinup period to the first day of the last month

if(Delta_Days($Syr, $Smon, 1, $Fyr, $Fmon, $Fday)+1 < $MinDays ||
            ($Syr==$Fyr && $Smon==$Fmon) ) {
  # calc percentiles for just one period
  @sy = ($Syr);   @sm = ($Smon);   @sd = (1);
  @ey = ($Fyr);   @em = ($Fmon);   @ed = ($Fday);
  @daycnt_thresh = (int($FractReq*Delta_Days($Syr,$Smon,1,$Fyr,$Fmon,$Fday)));
}
else {
  # calc percentiles for 2 periods; first is one month long
  ### Shrad added this ## 20120806
  ### The following line estimates the parameter of the second month of the spinup (i.e. the month before the current period)
  if (Delta_Days($Fyr, $Fmon, 1, $Fyr, $Fmon, $Fday)+1 < $MinDays)
  {($ty, $tm, $td) = Add_Delta_YM($Fyr, $Fmon, $Fday, 0, -1);
  @daycnt_thresh = (int($FractReq*Days_In_Month($Syr,$Smon)), int($FractReq*Days_In_Month($ty,$tm))+int($FractReq*$Fday));}
  else
  {($ty, $tm, $td) = ($Fyr, $Fmon, $Fday);
  @daycnt_thresh = (int($FractReq*Days_In_Month($Syr,$Smon)), int($FractReq*$Fday));}
  
  $td = 1;
  
  @sy = ($Syr,$ty);  @sm = ($Smon,$tm);  @sd = (1,1);
  @ey = ($Syr,$Fyr);  @em = ($Smon,$Fmon);
...
while(<PINF>) {
  ...
  ($curr_y[$r+1],$curr_m[$r+1],$curr_d[$r+1]) = Add_Delta_Days($ly,$lm,$ld,1);
...
for($s=0;$s<@ID;$s++) {
  ...
  open(RETRO, "<$RetroDir/$ID[$s]") or die "$0: ERROR: cannot open $RetroDir/$ID[$s]: $!\n";
  $r=0;
  while(<RETRO>) {
    if (/^\d/) {
...
#print "$s $per $sm[$per] $sd[$per] $em[$per] $ed[$per] $r $curr_m[$r] $curr_d[$r] $tmp_ptot $daycnt\n";
      # check for end of cal period(s)
...
#print "$s $per $sm[$per] $sd[$per] $tmp_ptot $daycnt $curr_ptot\n";
        $daycnt = 0;  # reset
      }
