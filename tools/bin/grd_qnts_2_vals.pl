...
#  @sy = ($Syr,$Fyr);  @sm = ($Smon,$Fmon);  @sd = (1,1);
#  @ey = ($Syr,$Fyr);  @em = ($Smon,$Fmon);  @ed = (Days_In_Month($Syr,$Smon),$Fday);
#  @daycnt = ( Delta_Days($Syr, $Smon, 1, $Syr, $Smon, Days_In_Month($Syr,$Smon))+1 ,
#              Delta_Days($Fyr, $Fmon, 1, $Fyr, $Fmon, $Fday)+1 );

  @sy = ($Syr,$S2yr);  @sm = ($Smon,$S2mon);  @sd = (1,1);
  @ey = ($Syr,$Fyr);  @em = ($Smon,$Fmon);  @ed = (Days_In_Month($Syr,$Smon),$Fday);
  @daycnt = ( Delta_Days($Syr, $Smon, 1, $Syr, $Smon, Days_In_Month($Syr,$Smon))+1 ,
...
for($c=0;$c<@flist;$c++) {
  ...
  open(RETRO, "<$RetroForcDir/$flist[$c]") or die "$0: ERROR: cannot open $RetroForcDir/$flist[$c]: $!\n";
  $r=0;
  while(<RETRO>) {
    ($p[$r],@tmp) = split;
    $r++;
  ...
  if($c==0) {
    ($y[0],$m[0],$d[0]) = ($ly,$lm,$ld) = ($RetroForcSyr, $RetroForcSmon, $RetroForcSday);
    for($n=0;$n<$r-1;$n++) {
  ...
  for($per=0;$per<@sm;$per++) {
    ...
    for($r=0;$r<@p;$r++) {
      if($y[$r] >= $Clim_Syr && $y[$r] <= $Clim_Eyr) { # in clim period
        ...
        if($m[$r]==$tmp_em && $d[$r]==$tmp_ed && $flag==1) { # end cal period
...
for($c=0;$c<@flist;$c++) {  # loop through cells
  ...
  # NOTE this is the transpose of other tabular datafiles used in this process
  # can facilitate plotting of percentiles if desired
...
# one 'upgrade' would be to add extrapolation, but that requires distrib. fitting
