...
sub run_vic {
  ...
  #      $metadata_template = "$ParamsModelDir/metadata.$output_prefix." .
  #        "template";
  #      $metadata = "$current_control_dir/metadata.$output_prefix.txt";
  #      if (!open(METADATA_TEMPLATE, "$metadata_template")) {
  #        $exit_code = 1;
  #        printf STDERR "$0: ERROR: cannot open metadata template " .
  #          "$metadata_template\n";
  #        return $exit_code;
  #      }
  ...
  #      close(METADATA);
  #      $cmd = "$TOOLS_DIR/vic2nc -i $results_dir_asc_daily " .
  #        "-p $output_prefix -m $metadata $gridfile_opt -o " .
  #        "$results_dir/$output_prefix $comp_str -t m >& $LOGFILE.tmp; " .
  ...
  ##      return $exit_code;
  #        printf STDERR "$0: WARNING: $cmd exited with error code: " .
  #          "$exit_code\n";
  #      }
  #    }
  #  }
...
sub run_noah {
  ...
  while ($year < $end_year || ($year == $end_year && $month <= $end_month)) {
    $old_state_file_name = sprintf "%s/state.%04d%02d.nc", $StateModelDir,
      $year, $month;
...
sub wrap_run_sac {
  ...
  # default pe dir will be the noah results directory for the same simulation
  # period, etc.
  $pe_dir = $results_dir;
  ...
  if ($extract_vars && $extract_vars !~ /^none$/i) {
    ...
    # $cmd = "$TOOLS_DIR/wrap_nc2vic.pl $results_dir $output_prefixes[0] " .
...
sub run_clm {
  ...
  ($clm_end_year, $clm_end_month, $clm_end_day) =
    Add_Delta_Days($end_year, $end_month, $end_day, 1);
  ...
  #    ">> $LOGFILE; exit";
  #  $cmd = "cd $MODEL_EXE_DIR; $MODEL_EXE_NAME >& $LOGFILE.tmp; " .
  ...
  $old_state_file_name = sprintf "%s/%s.clm2.r.%04d-%02d-%02d-00000.nc",
    $StateModelDir, $PROJECT, $clm_end_year, $clm_end_month, $clm_end_day;
  $new_state_file_name = sprintf "%s/state.%04d%02d%02d.nc", $StateModelDir,
