...
open (PARAMS_TEMPLATE, $Basin_control_file_Template)
  or die "$0: ERROR: cannot open parameter template file " .
...
open (BAS_CONTROLFILE, ">$BASIN_CONTROL_FILE")
  or die "$0: ERROR: cannot open current controlfile $BASIN_CONTROL_FILE\n";
foreach (@MyParamsInfo) {
  print BAS_CONTROLFILE;
}

#### Creating a basin control file for ESP forecasts and ENSO subsets
open (PARAMS_TEMPLATE, $ESP_control_file_Template)
  or die "$0: ERROR: cannot open parameter template file " .
...
open (ESP_CONTROLFILE, ">$ESP_CONTROL_FILE")
  or die "$0: ERROR: cannot open current controlfile $ESP_CONTROL_FILE\n";
foreach (@MyParamsInfo) {
  print ESP_CONTROLFILE;
}

### Creating Directories
foreach $dir ($ROUT_XYZZ_DIR, $ESP_XYZZ_DIR, $ROUT_PLOT_DIR, $ESP_PLOT_DIR,
              $LogDir, $ROUT_WEB_DIR) {
  $status = &make_dir($dir);
  if ($status != 0) {
...
$cmd = "$TOOLS_DIR/proc.xtr_vars.fcst.scr $FCST " .
...
$cmd = "$TOOLS_DIR/calc_fcst_stats.6var.fcst.scr " .
