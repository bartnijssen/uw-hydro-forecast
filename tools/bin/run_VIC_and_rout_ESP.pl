...
while ( $METYR <= $FEYR ) {    # forecast year loop
    ...
    ( $Stateyr, $Statemon, $Stateday ) =
      Add_Delta_Days( $Cyr, $Cmon, $Cday, 0 );
    $datestr = sprintf( "%04d%02d%02d", $Stateyr, $Statemon, $Stateday );

    #### Will have to change this for other models
    $LogFile =
      "$LogDir/log.$PROJECT.$MODEL.model_run.$datestr.$METYR.pl.$JOB_ID";
...
if ( exists $var_info_project{"EMAIL_LIST"}
    and $var_info_project{"EMAIL_LIST"} )
{
