...
while ($METYR <= $FEYR) {         # forecast year loop
  ...
  if ($RUN_ESP == 1) {
    ...
    $cmd = "$TOOLS_DIR/run_model_ESP.pl -m $MODEL -p $PROJECT -f retro " .
  ...
  if ($RUN_ROUT == 1) {
    ...
    $ROUT_END_DATE =
      sprintf "%04d-%02d-%02d", $Rout_Eyr, $Rout_Emon, $Rout_Eday;

    $cmd = "$TOOLS_DIR/run_rout_model.pl -m $MODEL -p $PROJECT " .
      "-f curr_spinup -r esp.$datestr.$FORC_START_DATE -s $ROUT_START_DATE " .
        "-e $ROUT_END_DATE -i $datestr -en $METYR -z $ESP_STORAGE " .
          ">& $LogFile.tmp";
    print "$cmd\n";
    (system($cmd)==0) or die "$0: ERROR: $cmd failed: $?\n";
    $cmd = "cat $LogFile.tmp >> $LogFile";
