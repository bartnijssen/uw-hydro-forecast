...
# number of usable days of data, and creates "fmt" file for input to gridding process.
#
# Author: Ted Bohn
# $Id: $
#-------------------------------------------------------------------------------
...
open (PFMT, $p_ndx_fmt_file) or die "$0: ERROR: cannot open p fmt file $p_ndx_fmt_file\n";
$UsableDays = 0;
foreach (<PFMT>) {
  chomp;
  @fields = split /\s+/;
...
print "Usable data in update period is from $Syr $Smon $Sday to $Fyr $Fmon $Fday\n";

# Check to see whether usable day is after existing last forcing day
$Update_Days = Delta_Days($Forc_Eyr, $Forc_Emon, $Forc_Eday, $Fyr, $Fmon, $Fday);

print "Number of update days since last update ($Forc_Eyr $Forc_Emon $Forc_Eday) is $Update_Days\n";
...
if($Update_Days <= 0) {
  ...
  print STDERR "No usable days since previous update on $Forc_Eyr $Forc_Emon $Forc_Eday found\n:";
  print STDERR "Quitting.  Check recent station data *.fmt files in $FmtDir for coverage\n";
