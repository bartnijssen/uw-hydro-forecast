...
#           percentile if $Fday>$MinDays, split update period in 2,
#           else lump together
#
# StnsReq - num of stns that must report for update (50%+ out of total number of
#           stations)
#
# FractReq - fraction of a percentile period that must be present for a given
#            station; otherwise assign void
#
...
open (FILE, $RetroStartDateFile)
  or die "$0: ERROR: cannot open file $RetroStartDateFile\n";
foreach (<FILE>) {
  if (/^(\d+)\s+(\d+)\s+(\d+)\s+/) {
...
open (FILE, $CurrspinStartDateFile)
  or die "$0: ERROR: cannot open file $CurrspinStartDateFile\n";
foreach (<FILE>) {
  if (/^(\d+)\s+(\d+)\s+(\d+)\s+/) {
    ($Syr,$Smon,$Sday) = ($1,$2,$3);
  }
...
open (FILE, $CurrspinEndDateFile)
  or die "$0: ERROR: cannot open file $CurrspinEndDateFile\n";
foreach (<FILE>) {
  if (/^(\d+)\s+(\d+)\s+(\d+)\s+/) {
    ($Eyr,$Emon,$Eday) = ($1,$2,$3);
  }
...
opendir (ACIS_DIR, $StnRawDir)
  or die "$0: ERROR: cannot open directory $StnRawDir for reading\n";
...
if (!$LastStnDateOverride) {
  opendir(STN_TS_DIR, $StnTSDir)
    or die "$0: ERROR: cannot open station timeseries directory " .
      "$SnTSDir for reading\n";
  ...
  if ($last_line =~ /^(\d\d\d\d)\s+(\d\d)\s+(\d\d)\s+/) {
    ...
    die "$0: ERROR: cannot determine date of last record in " .
      "station timeseries\n";
  }
} else {
  ($LastStnYear,$LastStnMon,$LastStnDay) = split /-/, $LastStnDateOverride;
}
($NextStnYear,$NextStnMon,$NextStnDay) = Add_Delta_Days($LastStnYear,
                                                        $LastStnMon,
                                                        $LastStnDay, 1);
...
$end_date = sprintf "%04d-%02d-%02d", $Fyr, $Fmon, $Fday;
  ...
  $cmd = "$TOOLS_DIR/update_fmt.pl $StnTSDir $p_ndx_fmt_file $tx_ndx_fmt_file " .
...
$cmd = "$TOOLS_DIR/calc.per_pcp_stn_qnts.pl $p_ndx_fmt_file $Syr $Smon " .
...
$cmd = "$TOOLS_DIR/run_regrid.pl $TOOLS_DIR $ConfigRegrid $template " .
  "$StnList $DEM $p_ndx_perqnt_fmt_file $p_perqnt_grd_file >& $LogFile.tmp; " .
  "cat $LogFile.tmp >> $LogFile";
print "$cmd\n";
(system($cmd)==0) or die "$0: ERROR: $cmd failed: $? \n";

# 3. transform gridded quantiles back to gridded period amounts
$cmd = "$TOOLS_DIR/grd_qnts_2_vals.pl $p_perqnt_grd_file $Syr $Smon " .
...
$cmd = "$TOOLS_DIR/grd_peramt_2_dlyamt.pl $p_peramt_grd_file " .
...
$cmd = "$TOOLS_DIR/stn_Tvals_2_anoms.pl $tx_ndx_fmt_file $tn_ndx_fmt_file " .
...
$cmd = "$TOOLS_DIR/run_regrid.pl $TOOLS_DIR $ConfigRegrid $template $StnList " .
...
$cmd = "$TOOLS_DIR/Tgrd_anoms_2_vals.pl $tx_anom_grd_file $tn_anom_grd_file " .
  "$MetMeansGrd $tx_grd_file $tn_grd_file >& $LogFile.tmp; " .
  "cat $LogFile.tmp >> $LogFile";
print "$cmd\n";
(system($cmd)==0) or die "$0: ERROR: $cmd failed: $?\n";
...
$cmd = "$TOOLS_DIR/grds_2_tser.pl $p_dlyamt_rsc_file $tx_grd_file " .
  "$tn_grd_file $Fyr $Fmon $Fday $Eyr $Emon $Eday $MetMeansGrd $CurrForcDir " .
  ">& $LogFile.tmp; cat $LogFile.tmp >> $LogFile";
print "$cmd\n";
(system($cmd)==0) or die "$0: ERROR: $cmd failed: $?\n";

# update the FORC.END_DATE file
open (END_DATE_TMP, ">$CurrspinEndDateFile.tmp")
  or die "$0: ERROR: cannot open end_date file $CurrspinEndDateFile.tmp " .
  "for writing\n";
