...
if (!$LastStnDateOverride) {
  opendir(STN_TS_DIR, $StnTSDir) or
    die "$0: ERROR: cannot open station timeseries directory " .
    "$SnTSDir for reading\n";
...
($NextStnYear, $NextStnMon, $NextStnDay) =
  Add_Delta_Days($LastStnYear, $LastStnMon, $LastStnDay, 1);
