...
# ---------- ARGS ------------------------------
if(@ARGV != 12) {
  die "Usage:  calc.ens_fcst_stats.pl <YYYY> <MM> <DD> >Clim_Syr> <Clim_Eyr> <Ens_Syr> <Ens_Eyr> <Flist> <NearRTDir> <RTDir> <EspDir> <Model>\n";
} else {
...
$Outfl  = "$EspDir/xyzz/$datestr/wb_qnts.ENS.$MODEL.$datestr.xyzz";
$Outfl2 = "$EspDir/xyzz/$datestr/wb_prob.ENS.20-33.$MODEL.$datestr.xyzz";
...
for($e=0;$e<$NENS;$e++) {
  ...
  $fname = $FutArchive . "fluxes." . ($Ens_Syr+$e) . "." . $basin . ".tar.gz";
  system("rm -R $dirname");
...
#    print "$c $cell[$c]\n";
    ...
    $name = $FutDir . "flux/$basin" . $cell[$c];
    
    if (!-e $name)
    {$name = $FutDir . "flux/asc/" . $cell[$c]; ##### flux files are in either FUTDIR/flux or FUTDIR/flux/asc
    }
  ...
  for($c=0;$c<$Ncells;$c++) {
    ...
    for($p=0;$p<@PER;$p++) {  # work backward, adding rest of periods
      ...
      for($r=$recbnd1b[$p];$r<$recbnd2[$p];$r++) {
        $cum_ro_3 += $ro[$c][$r];
	#print "Cumulative runoff is $cum_ro_3\n";
      }
...
foreach $col (@ROCols) 
   {
         $ro_col = $col;
         $ro_ref->[$c][$r] += $tmp[$ro_col];
   }
    foreach $col (@SMCols) {
         $sm_col = $col;
         $sm_ref->[$c][$r] += $tmp[$sm_col];
      }
    $swecol = $SWECol;
    $swe_ref->[$c][$r] = $tmp[$swecol]; # reference this way to pass back values
    $r++;
  }
  close(FL);
  $cnt=0;
