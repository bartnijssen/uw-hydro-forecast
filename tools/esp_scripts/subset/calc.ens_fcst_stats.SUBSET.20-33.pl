...
# LAST:  (1) could handle sm & swe separately, since don't need their spinup...
#             will cut down on memory needed
#         (2) also, could read only the part of the spinup fluxes needed to calculate 
#             the cumulative variables.

# ---------- ARGS ------------------------------
if(@ARGV != 3) {
  die "Usage:  calc.ens_fcst_stats.pl <YYYY> <MM> <DD>\n";
} else {
...
@SUBSET_YRS = (1955, 1956, 1957, 1965, 1968, 1971, 1972, 1974, 1975, 1976, 1984, 1985, 1989, 1996, 1999, 2000, 2001);  # for spring start
...
$Flist  = "/raid/forecast/common/CONUS.5.fluxfiles.maskorder";
$NearRTDir = "/raid/forecast/sw_monitor/output/spinup.05-nearRT/";
$RTDir = "/raid/forecast/sw_monitor/output/curr_spinup/";
#$FutDir = "/raid/forecast/sw_monitor/output/ESP/flux/";
$FutDir = "/raid/forecast/sw_monitor/output/ESP/temp/";
$FutArchive = "/raid/forecast/sw_monitor/output/ESP/summary/$datestr/";
# next file has sorted climatologies for three variables:  cum_ro, sm, swe
$ClimDist_Fl = "/raid/forecast/sw_monitor/output/ESP/xyzz.all/wb_vals.CLIM.$datestr.xyzz";
$Outfl = "/raid/forecast/sw_monitor/output/ESP/xyzz.all/wb_qnts.ENS.SUBSET.$datestr.xyzz";
$Outfl2 = "/raid/forecast/sw_monitor/output/ESP/xyzz.all/wb_prob.20-33.ENS.SUBSET.$datestr.xyzz";
...
for($e=0;$e<$NENS;$e++) {
  ...
  $fname = $FutArchive . "fluxes." . $SUBSET_YRS[$e] . ".tar.gz";
  system("rm -R $dirname");
