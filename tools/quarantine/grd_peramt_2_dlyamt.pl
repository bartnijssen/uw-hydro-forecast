...
if ($per == 1) {
  ...
  if ($per == 2) {
    ...
    @daycnt = (
               Delta_Days($Syr, $Smon, 1, $Syr, $Smon,
                          Days_In_Month($Syr, $Smon)) + 1,
               Delta_Days($S2yr, $S2mon, $S2day, $Fyr, $Fmon, $Fday) + 1
              );
  } else {
    print "Quitting. Per is not 1 or 2\n";
    die "$0: ERROR: $0 program being used incorrectly. Found other than 1 " .
      "or 2 periods worth of data in input files\n";
  }
}
