...
void add_sec_date(struct tm, int, struct tm *);
void Handle_Error(int);
int Handle_V_Option(struct OutputVar*,  char*);
int isPritableVariable(struct VarAtt*, struct OutputVar*, int);
...
{
  char *infile, *outpath, *prefix;
  char *filename, *metafile, *gridfile;
  FILE **outfilehandle;
  float **var3d;
  float ***var4d;
  int i, j, k, l, t, g;
  ...
  struct OutputVar outputVars[MAX_NUM_VARIABLES];
  int outputVarLen = 0;
  int startHandle = 0;
  ...
  //
  infile = (char*)calloc(150,sizeof(char));
  outpath = (char*)calloc(150,sizeof(char));
  prefix = (char*)calloc(150,sizeof(char));
  filename = (char*)calloc(150,sizeof(char));
  metafile = (char*)calloc(150,sizeof(char));
  gridfile = (char*)calloc(150,sizeof(char));

  var_atts = (struct VarAtt *)calloc(MAXVAR,sizeof(struct VarAtt));
  for(v=0; v< MAXVAR; v++) {
    var_atts[v].name = (char*)calloc(15,sizeof(char));
    var_atts[v].units = (char*)calloc(15,sizeof(char));
    var_atts[v].z_dep = (char*)calloc(5,sizeof(char));
    var_atts[v].sign = (char*)calloc(8,sizeof(char));
    var_atts[v].long_name = (char*)calloc(40,sizeof(char));
  }
  ...
  levels = (float*)calloc(NLEVELS,sizeof(float));
  ...
  var3d = (float**)calloc(NUM3d,sizeof(float*));
  if(NUM4d > 0) {
    var4d = (float***)calloc(NUM4d,sizeof(float**));
  }
  for (ov=0;ov<outputVarLen;ov++) {
    ...
    if (!strcasecmp(var_atts[v].z_dep,"FALSE")) {
      var3d[count3] = (float*)calloc(NCELLS,sizeof(float));
    }
    else {
      var4d[count4] = (float**)calloc(NCELLS,sizeof(float*));
      for (k=0; k<NCELLS; k++) {
        var4d[count4][k] = (float*)calloc(NLEVELS,sizeof(float));
      }
    }
  ...
  int *dtp, struct VarAtt *var_atts, struct OutputVar *outputVars, int *outputVarLen)
{
  ...
  temp_timestep = (int*)calloc(NRECS,sizeof(int));
...
//    Handle_Error(status);
  ...
  if ((status = nc_get_var_float(ncid, timevarid, temp_time)) == NC_NOERR) {
    for (t=0; t<NRECS; t++) {
      add_sec_date(origin_time, (int)temp_time[t], &(data_times[t]));
    }
  }
  else if ((status = nc_get_var_int(ncid, timestepvarid, temp_timestep)) == NC_NOERR) {
    // Handle case of fortran-style indexing
    if (temp_timestep[0] == 1 && *dtp != 1) {
      for (t=0; t<NRECS; t++) {
        temp_timestep[t]--;
      }
    }
    for (t=0; t<NRECS; t++) {
      add_sec_date(origin_time, (int)(temp_timestep[t]*(*dtp)), &(data_times[t]));
    }
  }
  else {
    for (t=0; t<NRECS; t++) {
      add_sec_date(origin_time, t*(*dtp), &(data_times[t]));
    }
  }
...
void Write_Metadata(char *metafile, char **global_atts, struct tm *data_times,
  ...
  start_datetime_str = (char *)calloc(19,sizeof(char));
  end_datetime_str = (char *)calloc(19,sizeof(char));
  file_start_date_str = (char *)calloc(10,sizeof(char));
  file_start_time_str = (char *)calloc(8,sizeof(char));
  file_end_date_str = (char *)calloc(10,sizeof(char));
  file_end_time_str = (char *)calloc(8,sizeof(char));
  file_start_datetime_str = (char *)calloc(19,sizeof(char));
  file_end_datetime_str = (char *)calloc(19,sizeof(char));
...
void Date_2_TM(char *date_str, struct tm *date_struct)
{
  ...
  for (i=0; i<4; i++) {
    words[i] = (char*)calloc(4,sizeof(char));
  }

  for (wordsp = words; (*wordsp = strsep(&date_str, "-")) != NULL;) {
    if (**wordsp != '\0') {
      if (++wordsp >= &words[4]) {
        break;
      }
  ...
  date_struct->tm_year = atoi(words[0]) - 1900;
  date_struct->tm_mon = atoi(words[1]) - 1;
  date_struct->tm_mday = atoi(words[2]);
...
void Time_2_TM(char *time_str, struct tm *date_struct)
{
  ...
  for (wordsp = words; (*wordsp = strsep(&time_str, ":")) != NULL;) {
    if (**wordsp != '\0') {
      if (++wordsp >= &words[4]) {
        break;
      }
  ...
  date_struct->tm_hour = atoi(words[0]);
  date_struct->tm_min = atoi(words[1]);
  date_struct->tm_sec = atoi(words[2]);
...
//*************************************************************************************************
void add_sec_date(struct tm start_date, int sec_to_add, struct tm *new_date) {
  ...
  while (ndays) {
    ...
    // Compute number of days required to increment year
    days_in_year = 365;
    if (LEAPYR(new_date->tm_year)) {
      // Leap year
      days_in_year++;
    }
    days_until_year_rollover = days_in_year - new_date->tm_mday + 1;
    for (mon=new_date->tm_mon-1; mon>=0; mon--) {
      days_until_year_rollover -= dmonth[mon];
      // Subtract extra day when we encounter February of a leap year
      if (LEAPYR(new_date->tm_year) && mon == 2) {
        days_until_year_rollover--;
      }
    }

    // Increment year if possible
    if (ndays >= days_until_year_rollover) {
      new_date->tm_year++;
      new_date->tm_mon = 0;
      new_date->tm_mday = 1;
      ...
      while (ndays) {
        ...
        // Compute number of days required to increment month
        ...
        // Increment month if possible
        if (ndays >= days_until_month_rollover) {
          new_date->tm_mon++;
          new_date->tm_mday = 1;
 ...
 */
int isPritableVariable(struct VarAtt* var_atts, struct OutputVar* outputVars, int length) {
        int ret = TRUE;
        int i=0;
        if (outputVars != NULL) {
                ret = FALSE;
