...
# the time series all contain the start date.
# Cmdline arguments
$stn_ts_dir = shift;
$stn_list   = shift;
$start_date = shift;
...
$nrecs_max = 0;
$first     = 1;
open(STNLIST, "$stn_list") or
  die "$0: ERROR: cannot open station list file $stn_list for reading\n";
STN_LOOP: foreach (<STNLIST>) {
  ...
  open(FILE, "$stn_ts_dir/$stn_data[$nstn]") or
    die "$0: ERROR: cannot open station data file " .
    "$stn_ts_dir/$stn_data[$nstn] for reading\n";
  $nrecs = 0;
  foreach $line (<FILE>) {
    chomp $line;
    if ($line =~ /^\d/) {
      ($year, $month, $day, $prcp, $tmax, $tmin) = split /\s+/, $line;
      if ($year * 1 == $start_year * 1 &&
          $month * 1 == $start_month * 1 &&
          $day * 1 == $start_day * 1) {
        $store = 1;
      }
      if ($store) {
...
# Open fmt files
open(PFMT, ">$pfmt") or
  die "$0: ERROR: cannot open p fmt file $pfmt for writing\n";
open(TXFMT, ">$txfmt") or
  die "$0: ERROR: cannot open tx fmt file $txfmt for writing\n";
open(TNFMT, ">$tnfmt") or
  die "$0: ERROR: cannot open tn fmt file $tnfmt for writing\n";
