...
$status = &GetOptions(\%options_hash,"h","m=s","p=s","f=s","s=s","e=s","r=s","i=s", "z=i","st=s","uncmp","l","x=s","mspc=s");
...
$var_info_project{"CONTROL_MODEL_DIR"}     =~ s/<CONTROL_SUBDIR>/esp/g; #### Control file and Log file for ESP runs go in to the subdirectory <esp>, seperated from nowcast runs
...
$STORDIR            = "$ESP/$MODEL_NAME/$FCST_DATE"; #### ESP FLUXOUTPUT storage directory 
...
if (!-e $STORDIR) 
{ $status = &make_dir($STORDIR);} ### Creating Storage directory
...
if ($post_process == 1)
{
#### The script which converts daily ESP flux output into monthly and also extracts variable for spatial plots
$cmd = "$TOOLS_DIR/FCST_Process_Scripts/xtr_monthly_ts.scr $start_year $PROJECT $TOOLS_DIR/FCST_Process_Scripts $STORDIR $results_dir_asc $STORDIR/MON.$start_year $Flist >& $LOGFILE.tmp; cat $LOGFILE.tmp >> $LOGFILE; rm $LOGFILE.tmp";
(($status = &shell_cmd($cmd,$LOGFILE)) == 0) or die "$0: ERROR: $cmd failed: $status\n";

####Now archive monthly flux output generated in last step
#### Directory $STORDIR/MON.$start_year where the monthly flux output for each ensemble (i.e. $start_year) gets stored. This directory is created in the script xtr_monthly_ts.scr

### Moving $STORDIR/MON.$start_year to current directory
$cmd = "mv  $STORDIR/MON.$start_year ./MON.$PROJECT.$start_year >& $LOGFILE.tmp; cat $LOGFILE.tmp >> $LOGFILE; rm $LOGFILE.tmp";
(($status = &shell_cmd($cmd,$LOGFILE)) == 0) or die "$0: ERROR: $cmd failed: $status\n >& $LOGFILE.tmp; cat $LOGFILE.tmp >> $LOGFILE; rm $LOGFILE.tmp";


$cmd = "tar -czf $STORDIR/monthly_flux/fluxes.mon.$start_year.tar.gz ./MON.$PROJECT.$start_year >& $LOGFILE.tmp; cat $LOGFILE.tmp >> $LOGFILE; rm $LOGFILE.tmp";
(($status = &shell_cmd($cmd,$LOGFILE)) == 0) or die "$0: ERROR: $cmd failed: $status\n";

$cmd = "rm -rf ./MON.$PROJECT.$start_year >& $LOGFILE.tmp; cat $LOGFILE.tmp >> $LOGFILE; rm $LOGFILE.tmp";
(($status = &shell_cmd($cmd,$LOGFILE)) == 0) or die "$0: ERROR: $cmd failed: $status\n";
...
$cmd = "mv  $results_dir_asc ./ESP.$PROJECT.$start_year >& $LOGFILE.tmp; cat $LOGFILE.tmp >> $LOGFILE; rm $LOGFILE.tmp";
(($status = &shell_cmd($cmd,$LOGFILE)) == 0) or die "$0: ERROR: $cmd failed: $status\n";

$cmd = "tar -czf $STORDIR/dly_flux/fluxes.$start_year.tar.gz ./ESP.$PROJECT.$start_year >& $LOGFILE.tmp; cat $LOGFILE.tmp >> $LOGFILE; rm $LOGFILE.tmp";
(($status = &shell_cmd($cmd,$LOGFILE)) == 0) or die "$0: ERROR: $cmd failed: $status\n";

$cmd = "rm -rf ./ESP.$PROJECT.$start_year >& $LOGFILE.tmp; cat $LOGFILE.tmp >> $LOGFILE; rm $LOGFILE.tmp";
(($status = &shell_cmd($cmd,$LOGFILE)) == 0) or die "$0: ERROR: $cmd failed: $status\n";

$cmd = "rm -rf $local_root"; ### Remove results directory esp for model which produce nc output
print "$cmd\n";
...
sub usage() {
  ...
  if ($_[0] eq "full") {
    ...
    print "\n";
    print "  -i <init_file>\n";
 "    (optional) Specify an initial state file.  This can be simply a file name, in which case the file\n";
