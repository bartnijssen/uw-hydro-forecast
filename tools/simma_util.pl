...
#-------------------------------------------------------------------------------
# read_config - reads a config file and returns a hash containing the key-value pairs
#
...
#                   repeated occurrences of a key result in the new value being appended
#                   to the old value with a ";;" separator
#-------------------------------------------------------------------------------
sub read_config {
  ...
  foreach (<CONFIG>) {
    ...
    if (!/^#/ && /\S/) {
      ...
      if (!$var_info{$key}) {
        ...
        # If this key has been encountered already, append the new value to the previous value with a ";;" as a separator
...
#   All other codes: these are the operating system error codes returned by the command
#-------------------------------------------------------------------------------
sub make_dir {
