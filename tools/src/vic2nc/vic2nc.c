...
void Usage(char *);
void Read_Metadata(char *, char **, struct tm *, struct tm *, float *, char *,
                   char *, char *, char *, float *, char *, char *,
                   struct VarAtt *);
...
void Compute_File_Time_Properties(char *, struct tm *, struct tm *, struct tm *,
                                  int, char *, int, int, struct tm *,
                                  struct tm *);
void Read_VIC(struct GridInfo *, FILE **, struct VarAtt *, float **, float ***);
void Open_NetCDF(char *, char *, char *, char *, char *, char *, char *, float,
                 struct VarAtt *, char **, struct GridInfo *, float *, int *,
                 int *,
                 int *);
void Write_NetCDF(int, struct GridInfo *, int, int, struct VarAtt *, int, float,
                  int, float **,
                  float ***);
     ...
     char *argv[])
{
    ...
    Read_Args(argc, argv, inpath, prefix, metafile, outprefix, gridfile,
              timespan);

    // Read the metadata file
    Read_Metadata(metafile, global_atts, data_start_time, data_end_time, &dt,
                  time_units, time_units_str, timestep_units_str, lev_units,
                  levels,
                  lev_long_name, lev_positive,
                  var_atts);
    ...
    fprintf(stdout, "End date: %04d-%02d\n", data_end_time->tm_year + 1900,
            data_end_time->tm_mon + 1);
    ...
    if (NUM4d > 0) {
        ...
        for (v = 0; v < NUM4d; v++) {
            ...
            for (k = 0; k < NCELLS; k++) {
                if ((var4d[v][k] =
                         (float*)calloc(NLEVELS, sizeof(float))) == NULL) {
                    fprintf(
                        stderr,
                        "ERROR: cannot allocate sufficient memory for data array\n");
                    exit(1);
    ...
    for (fidx = 0; fidx < NFILES; fidx++) {
        ...
        Compute_File_Time_Properties(timespan, file_start_time, data_end_time,
                                     first_rec_time, dt, time_units, NFILES,
                                     fidx, file_end_time,
                                     last_rec_time);
        ...
        if (!strcasecmp(timespan, "total")) {
            ...
            sprintf(outfile, "%s.%04d.nc", outprefix,
                    file_start_time->tm_year + 1900);
        }
        else if (!strcasecmp(timespan, "m")) {
            sprintf(outfile, "%s.%04d%02d.nc", outprefix,
                    file_start_time->tm_year + 1900,
                    file_start_time->tm_mon + 1);
        }
        fprintf(stdout, "Writing to file %s\n", outfile);
        Open_NetCDF(outfile, lev_units, lev_long_name, lev_positive,
                    time_units_str,
                    file_start_time_str, timestep_units_str, dt, var_atts,
                    global_atts, grid, levels, &ncid, &timevarid,
                    &timestepvarid);
        ...
        for (t = 0; t < NRECS; t++) {
            ...
            if (!(LEAPYR(curr_rec_time->tm_year + 1900) &&
                  curr_rec_time->tm_mon == 1 && curr_rec_time->tm_mday ==
                  29) ||
                !NO_LEAP_OUT) {
                Write_NetCDF(ncid, grid, timevarid, timestepvarid, var_atts, t,
                             time, timestep, var3d,
                             var4d);
            }
...
Usage(char *temp)
{
    fprintf(
        stderr,
        "%s - converts traditional VIC forcing or results files into netcdf format.\n",
        temp);
    fprintf(stderr, "\n");
    fprintf(
        stderr,
        "Usage: %s  -i<inpath> -p<prefix> -m<metadata> -o<outfile> [-g<gridfile>] [-t<timespan>] [-c] [-l] [-L]\n",
        temp);
    fprintf(stderr, "\n");
    fprintf(
        stderr,
        "This program takes as input a set of VIC-format data files, 1 per grid cell,\n");
    fprintf(
        stderr,
        "and writes their contents into 1 netcdf file.  The VIC-format data files must\n");
    fprintf(
        stderr,
        "be ascii text files, consisting of one or more lines of space-separated fields.\n");
    fprintf(stderr, "\n");
    fprintf(
        stderr,
        "In addition to specifying the location of the input files, you must supply a\n");
    fprintf(
        stderr,
        "metadata file (the \'metafile\') containing the names and order of the variables\n");
    fprintf(stderr, "in the input files and the overall data dimensions.\n");
    fprintf(stderr, "\n");
    fprintf(
        stderr,
        "By default, this program assumes that the grid\'s rows and columns are aligned\n");
    fprintf(
        stderr,
        "with lines of latitude and longitude.  It uses the latitude and longitude values\n");
    fprintf(
        stderr,
        "in the VIC data file names to determine the row and column indices of the position\n");
    fprintf(stderr, "in the netcdf array where the data will be stored.\n");
    fprintf(stderr, "\n");
    fprintf(
        stderr,
        "However, if you are working with an equal-area grid, in which latitude and longitude\n");
    fprintf(
        stderr,
        "may not be constant along a row or column, you can tell the program to get the row and\n");
    fprintf(
        stderr,
        "column of each grid cell from a separate file, called the \'gridfile\'.\n");
    fprintf(stderr, "\n");
    ...
    fprintf(
        stderr,
        "    <prefix>   Filename prefix of VIC files.  VIC data files must have names of\n");
    fprintf(
        stderr,
        "               form prefix_lat_lon, where \'lat\' and \'lon\' are the latitude and\n");
    fprintf(
        stderr,
        "               longitude of the grid cell and \'prefix\' is some character string.\n");
    fprintf(stderr, "\n");
    fprintf(stderr, "  -m <metadata>\n");
    fprintf(
        stderr,
        "    <metadata> Text file containing metadata for VIC files, including start/end\n");
    fprintf(
        stderr,
        "               times, time step, vertical levels, variables and their attributes.\n");
    fprintf(stderr, "\n");
    ...
    fprintf(
        stderr,
        "    (Optional) If -g is specified, the row and column values in the given gridfile\n");
    fprintf(
        stderr,
        "    will be used to index the grid cells in the netcdf file, instead of calculating the\n");
    fprintf(
        stderr,
        "    row and column from the lat and lon of the grid cells.  This option is useful when\n");
    fprintf(
        stderr,
        "    working with an equal-area grid, in which the rows and columns are not necessarily\n");
    fprintf(stderr, "    aligned with lines of latitude and longitude.\n");
    fprintf(
        stderr,
        "    <gridfile> Text file with id#, row, col, lat, lon of grid cells.  Format:\n");
    fprintf(stderr, "                 CellID Row Col Lat Lon\n");
    fprintf(
        stderr,
        "               where CellID = ID number of the cell, Row and Col are the cell\'s row\n");
    fprintf(
        stderr,
        "               and column indexes, and Lat and Lon are the cell\'s latitude and\n");
    fprintf(stderr, "               longitude.\n");
    fprintf(
        stderr,
        "    Note: when -g is specified, \'row\', \'col\', and \'cellid\' will be stored in the\n");
    fprintf(stderr, "    netcdf file\n");
    fprintf(
        stderr,
        "    Default: if -g is NOT specified, row and col indexes will be determined from the\n");
    fprintf(
        stderr,
        "    grid cell lat and lon values.  In addition, \'row\', \'col\', and \'cellid\' will\n");
    fprintf(stderr, "    not be stored in the netcdf file.\n");
    fprintf(stderr, "\n");
    fprintf(stderr, "  -t <timespan>\n");
    fprintf(
        stderr,
        "    (Optional) If -t is specified, multiple output files will be created, each spanning\n");
    ...
    fprintf(
        stderr,
        "    The output filenames will be of the format <outfile>.<date>.nc, where <outfile> is\n");
    fprintf(
        stderr,
        "    the string specified with the -o option above, and <date> is the date of the first\n");
    fprintf(
        stderr,
        "    record in the file.  If <timespan> \'m\', <date> will be YYYYMM, where YYYY = year\n");
    fprintf(
        stderr,
        "    and MM = month.  If <timespan> is \'y\' <date> will be YYYY.\n");
    fprintf(stderr, "\n");
    fprintf(stderr, "  -c\n");
    fprintf(
        stderr,
        "    (Optional) If specified, use compression by gathering to save space.  This will\n");
    fprintf(
        stderr,
        "    cause the netcdf file\'s \'x\' and \'y\' dimensions to be replaced by a single\n");
    fprintf(stderr, "    \'land\' dimension.\n");
    fprintf(stderr, "\n");
    fprintf(stderr, "  -l\n");
    fprintf(
        stderr,
        "    (Optional) If specified, this option assumes that the input files DO contain February\n");
    fprintf(
        stderr,
        "    29 during leap years, but does NOT write Feb 29 to the output files.\n");
    fprintf(stderr, "\n");
}
...
// time_t *tp;
    ...
    char       endian_str[10];
    long long  dsec;
    const char year_unit_strs[][10] = {
        "year", "years", "yr", "yrs", "y", "ys", "yearly", "", ""
    };
    const char month_unit_strs[][10] = {
        "month", "months", "mon", "mons", "mo", "mos", "m", "ms", "monthly"
    };
    const char day_unit_strs[][10] = {
        "day", "days", "dy", "dys", "d", "ds", "daily", "", ""
    };
    const char hour_unit_strs[][10] = {
        "hour", "hours", "hr", "hrs", "h", "hs", "hourly", "", ""
    };
    const char min_unit_strs[][10] = {
        "minute", "minutes", "min", "mins", "mi", "mis", "mn", "mns",
        "minutely"
    };
    const char sec_unit_strs[][10] = {
        "second", "seconds", "sec", "secs", "s", "ss", "secondly", "", ""
    };
    ...
    while (!feof(mf)) {
        if (cmdstr[0] != '#' && cmdstr[0] != '\n' && cmdstr[0] != '\0') {
            ...
            if (strcasecmp("INSTITUTION", optstr) == 0) {
                ...
                sscanf(cmdstr, "%*s %s %s", data_start_date_str,
                       data_start_time_str);
            }
            else if (strcasecmp("END_TIME", optstr) == 0) {
                sscanf(cmdstr, "%*s %s %s", data_end_date_str,
                       data_end_time_str);
            }
                ...
                if (NLEVELS == 3) {
                    sscanf(cmdstr, "%*s %f %f %f", &levels[0], &levels[1],
                           &levels[2]);
                }
            }
            ...
            else if (!strcasecmp("TRUE",
                                 optstr) || !strcasecmp("FALSE", optstr)) {
                sscanf(cmdstr, "%s %s %s %s %s %s %[^\n]",
                       var_atts[varcount].read, var_atts[varcount].name,
                       var_atts[varcount].units,
                       var_atts[varcount].z_dep, var_atts[varcount].sign,
                       multstr,
                       var_atts[varcount].long_name);
                ...
                if (!strcasecmp(var_atts[varcount].z_dep,
                                "TRUE") && NLEVELS == 0) {
                    fprintf(
                        stderr,
    ...
    if (NCELLS > 1) {
        ...
        for (k = 0; k < NCELLS; k++) {
            if (first_time) {
                ...
                if (fabs(lats[k] - temp) >= 0.0001 && delta_lat >
                    fabs(lats[k] - temp)) {
        ...
        for (k = 0; k < NCELLS; k++) {
            if (first_time) {
                ...
                if (fabs(lons[k] - temp) >= 0.0001 && delta_lon >
                    fabs(lons[k] - temp)) {
                ...
                FILE           **infilehandle)
{
    ...
    for (k = 0; k < NCELLS; k++) {
        ...
        sprintf(filename, "%s/%s_%.4f_%.4f", inpath, prefix, grid[k].lat,
                grid[k].lon);
...
// *************************************************************************************************
void
Compute_NFILES(struct tm *data_start_time,
               struct tm *data_end_time,
               char      *timespan)
{
    long long tmp;
...
// *************************************************************************************************
void
Compute_File_Time_Properties(char      *timespan,
                             struct tm *file_start_time,
                             struct tm *data_end_time,
                             struct tm *first_rec_time,
                             int        dt,
                             ...
                             struct tm *last_rec_time)
{
    ...
    if (!strcmp(timespan, "total") || fidx == NFILES - 1) {
        ...
        if (!strcmp(timespan, "y")) {
            ndays = 364;
            if ((LEAPYR(file_start_time->tm_year + 1900) &&
                 (file_start_time->tm_mon == 0 ||
                  (file_start_time->tm_mon == 1 && file_start_time->tm_mday <=
             28))) ||
                (LEAPYR(file_start_time->tm_year + 1900 + 1) &&
                 file_start_time->tm_mon == 2)) {
                ndays++;
            }
        }
        else if (!strcmp(timespan, "m")) {
       ...
       char     *time_units)
{
    ...
    if (!strcmp(time_units, "year")) {
        ...
        for (i = 0; i < dt; i++) {
            rec_time.tm_year++;
            ndays += 365;
            if ((LEAPYR(rec_time.tm_year + 1900 - 1) &&
                 (rec_time.tm_mon == 0 ||
                  (rec_time.tm_mon == 1 && rec_time.tm_mday <= 28))) ||
        ...
        for (i = 0; i < dt; i++) {
            ...
            if (LEAPYR(rec_time.tm_year + 1900) &&
                (rec_time.tm_mon - 1 == 1 && rec_time.tm_mday <= 28)) {
                ndays++;
            }
            ...
            for (v = 0; v < (NUM3d + NUM4d); v++) {
                if (!strcasecmp(var_atts[v].z_dep, "FALSE")) {
                    if (!strcasecmp(FORMAT, "SCIENTIFIC")) {
                        if (fscanf(infilehandle[k], "%e",
                                   &(var3d[count3][k])) == EOF) {
                            var3d[count3][k] = NODATA_FLOAT;
                        }
                    }
                    else {
                        if (fscanf(infilehandle[k], "%f",
                                   &(var3d[count3][k])) == EOF) {
                            var3d[count3][k] = NODATA_FLOAT;
                        }
                    }
...
// *************************************************************************************************
void
Open_NetCDF(char            *outfile,
            char            *lev_units,
            char            *lev_long_name,
            char            *lev_positive,
            char            *time_units_str,
            char            *file_start_time_str,
            char            *timestep_units_str,
            float            dt,
            struct VarAtt   *var_atts,
            char           **global_atts,
...
// fprintf(stderr,"Defining netcdf file\n");
    status = nc_create(outfile, NC_CLOBBER, ncidp);
    Handle_Error(status);
    ncid = *ncidp;

    // Define Dimensions
    if (COMPRESS) {
        status = nc_def_dim(ncid, "land", NCELLS, &landdimid);
        Handle_Error(status);
    }
    else {
        status = nc_def_dim(ncid, "x", NCOLS, &xdimid);
        Handle_Error(status);
        status = nc_def_dim(ncid, "y", NROWS, &ydimid);
        Handle_Error(status);
    }
    if (NUM4d > 0) {
        status = nc_def_dim(ncid, "z", NLEVELS, &zdimid);
        Handle_Error(status);
    }
    status = nc_def_dim(ncid, "tstep", NC_UNLIMITED, &tstepdimid);
    Handle_Error(status);
...
// fprintf(stderr,"Done defining dimensions\n");
    ...
    if (GRIDFILE) {
        // Row
        status = nc_def_var(ncid, "row", NC_INT, ndim, dims, &rowvarid);
        Handle_Error(status);
        status = nc_put_att_text(ncid, rowvarid, "axis", strlen("YX"), "YX");
        Handle_Error(status);
        status = nc_put_att_text(ncid, rowvarid, "units", strlen("-"), "-");
        Handle_Error(status);
        tempint = MINROW;
        status = nc_put_att_int(ncid, rowvarid, "valid_min", NC_INT, 1,
                                &tempint);
        Handle_Error(status);
        tempint = MAXROW;
        status = nc_put_att_int(ncid, rowvarid, "valid_max", NC_INT, 1,
                                &tempint);
        Handle_Error(status);
        status = nc_put_att_text(ncid, rowvarid, "long_name", strlen(
                                     "Grid Row"), "Grid Row");
        Handle_Error(status);
        if (COMPRESS) {
            status =
                nc_put_att_text(ncid, rowvarid, "associate", strlen(
                                    "(row col)"), "(row col)");
            Handle_Error(status);
        }
        else {
        ...
        tempint = NODATA_INT;
        status = nc_put_att_int(ncid, rowvarid, "missing_value", NC_INT, 1,
                                &tempint);
        Handle_Error(status);

        // Col
        status = nc_def_var(ncid, "col", NC_INT, ndim, dims, &colvarid);
        Handle_Error(status);
        status = nc_put_att_text(ncid, colvarid, "axis", strlen("YX"), "YX");
        Handle_Error(status);
        status = nc_put_att_text(ncid, colvarid, "units", strlen("-"), "-");
        Handle_Error(status);
        tempint = MINCOL;
        status = nc_put_att_int(ncid, colvarid, "valid_min", NC_INT, 1,
                                &tempint);
        Handle_Error(status);
        tempint = MAXCOL;
        status = nc_put_att_int(ncid, colvarid, "valid_max", NC_INT, 1,
                                &tempint);
        Handle_Error(status);
        status = nc_put_att_text(ncid, colvarid, "long_name", strlen(
                                     "Grid Col"), "Grid Col");
        Handle_Error(status);
        if (COMPRESS) {
            status =
                nc_put_att_text(ncid, colvarid, "associate", strlen(
                                    "(row col)"), "(row col)");
            Handle_Error(status);
        }
        else {
        ...
        tempint = NODATA_INT;
        status = nc_put_att_int(ncid, colvarid, "missing_value", NC_INT, 1,
                                &tempint);
        Handle_Error(status);
    }

    // Latitude
    status = nc_def_var(ncid, "nav_lat", NC_FLOAT, ndim, dims, &latvarid);
    Handle_Error(status);
    status = nc_put_att_text(ncid, latvarid, "axis", strlen("YX"), "YX");
    Handle_Error(status);
    status = nc_put_att_text(ncid, latvarid, "units", strlen(
                                 "degrees_north"), "degrees_north");
    Handle_Error(status);
    tempfloat = -90.0;
    status = nc_put_att_float(ncid, latvarid, "valid_min", NC_FLOAT, 1,
                              &tempfloat);
    Handle_Error(status);
    tempfloat = 90.0;
    status = nc_put_att_float(ncid, latvarid, "valid_max", NC_FLOAT, 1,
                              &tempfloat);
    Handle_Error(status);
    status = nc_put_att_text(ncid, latvarid, "long_name", strlen(
                                 "Latitude"), "Latitude");
    Handle_Error(status);
    if (COMPRESS) {
        status =
            nc_put_att_text(ncid, latvarid, "associate", strlen(
                                "(row col)"), "(row col)");
        Handle_Error(status);
    }
    else {
        status = nc_put_att_text(ncid, latvarid, "associate", strlen(
                                     "row col"), "row col");
        Handle_Error(status);
    }
    tempfloat = NODATA_FLOAT;
    status = nc_put_att_float(ncid, latvarid, "missing_value", NC_FLOAT, 1,
                              &tempfloat);
    Handle_Error(status);

    // Longitude
    status = nc_def_var(ncid, "nav_lon", NC_FLOAT, ndim, dims, &lonvarid);
    Handle_Error(status);
    status = nc_put_att_text(ncid, lonvarid, "axis", strlen("YX"), "YX");
    Handle_Error(status);
    status = nc_put_att_text(ncid, lonvarid, "units", strlen(
                                 "degrees_east"), "degrees_east");
    Handle_Error(status);
    tempfloat = -180.0;
    status = nc_put_att_float(ncid, lonvarid, "valid_min", NC_FLOAT, 1,
                              &tempfloat);
    Handle_Error(status);
    tempfloat = 180.0;
    status = nc_put_att_float(ncid, lonvarid, "valid_max", NC_FLOAT, 1,
                              &tempfloat);
    Handle_Error(status);
    status = nc_put_att_text(ncid, lonvarid, "modulo", strlen("360.0"), "360.0");
    Handle_Error(status);
    status = nc_put_att_text(ncid, lonvarid, "topology", strlen(
                                 "circular"), "circular");
    Handle_Error(status);
    status = nc_put_att_text(ncid, lonvarid, "long_name", strlen(
                                 "Longitude"), "Longitude");
    Handle_Error(status);
    if (COMPRESS) {
        status =
            nc_put_att_text(ncid, lonvarid, "associate", strlen(
                                "(row col)"), "(row col)");
        Handle_Error(status);
    }
    else {
        status = nc_put_att_text(ncid, lonvarid, "associate", strlen(
                                     "row col"), "row col");
        Handle_Error(status);
    }
    tempfloat = NODATA_FLOAT;
    status = nc_put_att_float(ncid, lonvarid, "missing_value", NC_FLOAT, 1,
                              &tempfloat);
    Handle_Error(status);

    // Landmask
    status = nc_def_var(ncid, "land", NC_INT, ndim, dims, &landvarid);
    Handle_Error(status);
    if (COMPRESS) {
        status = nc_put_att_text(ncid, landvarid, "compress", strlen(
                                     "y x"), "y x");
        Handle_Error(status);
    }
    else {
        status = nc_put_att_text(ncid, landvarid, "axis", strlen("YX"), "YX");
        Handle_Error(status);
        status =
            nc_put_att_text(ncid, landvarid, "units", strlen(
                                "0=invalid, 1=valid"), "0=invalid, 1=valid");
        Handle_Error(status);
        status = nc_put_att_text(ncid, landvarid, "associate", strlen(
                                     "row col"), "row col");
        Handle_Error(status);
    }
    status = nc_put_att_text(ncid, landvarid, "long_name", strlen(
                                 "Land Mask"), "Land Mask");
    Handle_Error(status);
    ...
    if (NUM4d > 0) {
        ndim = 1;
        dims[0] = zdimid;
        status = nc_def_var(ncid, "level", NC_FLOAT, ndim, dims, &levvarid);
        Handle_Error(status);
        status = nc_put_att_text(ncid, levvarid, "units", strlen(
                                     lev_units), lev_units);
        Handle_Error(status);
        status =
            nc_put_att_text(ncid, levvarid, "positive", strlen(
                                lev_positive), lev_positive);
        Handle_Error(status);
        tempfloat = LEVMIN;
        status = nc_put_att_float(ncid, levvarid, "valid_min", NC_FLOAT, 1,
                                  &tempfloat);
        Handle_Error(status);
        tempfloat = LEVMAX;
        status = nc_put_att_float(ncid, levvarid, "valid_max", NC_FLOAT, 1,
                                  &tempfloat);
        Handle_Error(status);
        status =
            nc_put_att_text(ncid, levvarid, "long_name", strlen(
                                lev_long_name), lev_long_name);
        Handle_Error(status);
    }
    ...
    ndim = 1;
    dims[0] = tstepdimid;
    status = nc_def_var(ncid, "time", NC_FLOAT, ndim, dims, timevaridp);
    Handle_Error(status);
    status = nc_put_att_text(ncid, *timevaridp, "units", strlen(
                                 time_units_str), time_units_str);
    Handle_Error(status);
    status = nc_put_att_text(ncid, *timevaridp, "calendar", strlen(
                                 "gregorian"), "gregorian");
    Handle_Error(status);
    status = nc_put_att_text(ncid, *timevaridp, "title", strlen("Time"), "Time");
    Handle_Error(status);
    status = nc_put_att_text(ncid, *timevaridp, "long_name", strlen(
                                 "Time axis"), "Time axis");
    Handle_Error(status);
    status =
        nc_put_att_text(ncid, *timevaridp, "origin", strlen(
                            file_start_time_str), file_start_time_str);
    Handle_Error(status);

    // Timestep
    ndim = 1;
    dims[0] = tstepdimid;
    status = nc_def_var(ncid, "timestp", NC_INT, ndim, dims, timestepvaridp);
    Handle_Error(status);
    status =
        nc_put_att_text(ncid, *timestepvaridp, "units", strlen(
                            timestep_units_str), timestep_units_str);
    Handle_Error(status);
    status =
        nc_put_att_text(ncid, *timestepvaridp, "title", strlen(
                            "Time Steps"), "Time Steps");
    Handle_Error(status);
    tempint = (int)dt;
    status = nc_put_att_int(ncid, *timestepvaridp, "tstep_sec", NC_INT, 1,
                            &tempint);
    Handle_Error(status);
    status =
        nc_put_att_text(ncid, *timestepvaridp, "long_name", strlen(
                            "Time step axis"), "Time step axis");
    Handle_Error(status);
    status =
        nc_put_att_text(ncid, *timestepvaridp, "origin", strlen(
                            file_start_time_str), file_start_time_str);
    Handle_Error(status);
...
// fprintf(stderr,"Done defining coordinate variables\n");
    ...
    status = nc_def_var(ncid, "CellID", NC_INT, ndim, dims, &cellidvarid);
    Handle_Error(status);
    status = nc_put_att_text(ncid, cellidvarid, "axis", strlen("YX"), "YX");
    Handle_Error(status);
    status = nc_put_att_text(ncid, cellidvarid, "long_name", strlen(
                                 "Cell ID"), "Cell ID");
    Handle_Error(status);
    if (COMPRESS) {
        status =
            nc_put_att_text(ncid, cellidvarid, "associate", strlen(
                                "(row col)"), "(row col)");
        Handle_Error(status);
    }
    else {
    ...
    tempint = NODATA_INT;
    status = nc_put_att_int(ncid, cellidvarid, "missing_value", NC_INT, 1,
                            &tempint);
    Handle_Error(status);
    ...
    for (v = 0; v < (NUM3d + NUM4d); v++) {
        if (!strcasecmp(var_atts[v].read, "TRUE")) {
            if (!strcasecmp(var_atts[v].z_dep, "FALSE")) {
                ...
                status =
                    nc_def_var(ncid, var_atts[v].name, NC_FLOAT, ndim, dims,
                               &(var_atts[v].id));
                Handle_Error(status);
                status =
                    nc_put_att_text(ncid, var_atts[v].id, "axis", strlen(
                                        "TYX"), "TYX");
                Handle_Error(status);
                status =
                    nc_put_att_text(ncid, var_atts[v].id, "units",
                                    strlen(
                                        var_atts[v].units), var_atts[v].units);
                Handle_Error(status);
                status =
                    nc_put_att_text(ncid, var_atts[v].id, "long_name", strlen(
                                        var_atts[v].long_name),
                                    var_atts[v].long_name);
                Handle_Error(status);
                if (COMPRESS) {
                    status =
                        nc_put_att_text(ncid, var_atts[v].id, "associate",
                                        strlen(
                                            "time (row col)"), "time (row col)");
                    Handle_Error(status);
                }
                else {
                    status =
                ...
                tempfloat = NODATA_FLOAT;
                status =
                    nc_put_att_float(ncid, var_atts[v].id, "missing_value",
                                     NC_FLOAT,
                                     1,
                                     &tempfloat);
                Handle_Error(status);
...
// fprintf(stderr,"Done defining data variables\n");
    ...
    status = nc_put_att_text(ncid, NC_GLOBAL, "file_name", strlen(
                                 outfile), outfile);
    Handle_Error(status);
    status = nc_put_att_text(ncid, NC_GLOBAL, "Conventions", strlen(
                                 "GDT 1.2"), "GDT 1.2");
    Handle_Error(status);
    status =
        nc_put_att_text(ncid, NC_GLOBAL, "institution", strlen(
                            global_atts[0]), global_atts[0]);
    Handle_Error(status);
    status = nc_put_att_text(ncid, NC_GLOBAL, "sources", strlen(
                                 global_atts[1]), global_atts[1]);
    Handle_Error(status);
    status =
        nc_put_att_text(ncid, NC_GLOBAL, "production", strlen(
                            global_atts[2]), global_atts[2]);
    Handle_Error(status);
    status = nc_put_att_text(ncid, NC_GLOBAL, "history", strlen(
                                 global_atts[3]), global_atts[3]);
    Handle_Error(status);
    status =
        nc_put_att_text(ncid, NC_GLOBAL, "projection", strlen(
                            global_atts[4]), global_atts[4]);
    Handle_Error(status);
    status =
        nc_put_att_text(ncid, NC_GLOBAL, "SurfSgn_convention", strlen(
                            global_atts[5]), global_atts[5]);
    Handle_Error(status);
...
// fprintf(stderr,"Done defining global attributes\n");
    ...
    if (GRIDFILE) {
        ...
        if (COMPRESS) {
            ...
            status = nc_put_var_int(ncid, rowvarid, temp_array_int_cmp);
            Handle_Error(status);
        }
        else {
            ...
            status = nc_put_var_int(ncid, rowvarid, temp_array_int);
            Handle_Error(status);
        }
        ...
        if (COMPRESS) {
            ...
            status = nc_put_var_int(ncid, colvarid, temp_array_int_cmp);
            Handle_Error(status);
        }
        else {
            // No compression; pad invalid cells with NODATA
            ...
            status = nc_put_var_int(ncid, colvarid, temp_array_int);
            Handle_Error(status);
        }
    }
    ...
    if (COMPRESS) {
        ...
        status = nc_put_var_float(ncid, latvarid, temp_array_cmp);
        Handle_Error(status);
    }
    else {
        // No compression; pad invalid cells with NODATA
        ...
        status = nc_put_var_float(ncid, latvarid, temp_array);
        Handle_Error(status);
    }
    ...
    if (COMPRESS) {
        ...
        status = nc_put_var_float(ncid, lonvarid, temp_array_cmp);
        Handle_Error(status);
    }
    else {
        // No compression; pad invalid cells with NODATA
        ...
        status = nc_put_var_float(ncid, lonvarid, temp_array);
        Handle_Error(status);
    }
    ...
    if (COMPRESS) {
        ...
        status = nc_put_var_int(ncid, landvarid, temp_array_int_cmp);
        Handle_Error(status);
    }
    else {
        ...
        status = nc_put_var_int(ncid, landvarid, temp_array_int);
        Handle_Error(status);
    }

    // Level
    if (NUM4d > 0) {
        status = nc_put_var_float(ncid, levvarid, levels);
    }
    Handle_Error(status);
    ...
    if (COMPRESS) {
        ...
        status = nc_put_var_int(ncid, cellidvarid, temp_array_int_cmp);
        Handle_Error(status);
    }
    else {
        // No compression; pad invalid cells with NODATA
        ...
        status = nc_put_var_int(ncid, cellidvarid, temp_array_int);
        Handle_Error(status);
    }
             ...
             float         ***var4d)
{
    ...
    start1d[0] = t_index;
    count1d[0] = 1;
    status = nc_put_vara_float(ncid, timevarid, start1d, count1d, &time);
    Handle_Error(status);
    status = nc_put_vara_int(ncid, timestepvarid, start1d, count1d, &timestep);
    Handle_Error(status);
        ...
        for (v = 0; v < (NUM3d + NUM4d); v++) {
            if (!strcasecmp(var_atts[v].read, "TRUE")) {
                if (!strcasecmp(var_atts[v].z_dep, "FALSE")) {
                    ...
                    status =
                        nc_put_vara_float(ncid, var_atts[v].id, start3d_cmp,
                                          count3d_cmp,
                                          temp_array_cmp);
                    Handle_Error(status);
                    varcount3++;
                    ...
                    for (l = 0; l < NLEVELS; l++) {
                        ...
                        status =
                            nc_put_vara_float(ncid, var_atts[v].id, start4d_cmp,
                                              count4d_cmp,
                                              temp_array_cmp);
                        Handle_Error(status);
                    }
        ...
        for (v = 0; v < (NUM3d + NUM4d); v++) {
            if (!strcasecmp(var_atts[v].read, "TRUE")) {
                if (!strcasecmp(var_atts[v].z_dep, "FALSE")) {
                    ...
                    count3d[2] = NCOLS;
                    status =
                        nc_put_vara_float(ncid, var_atts[v].id, start3d,
                                          count3d,
                                          temp_array);
                    Handle_Error(status);
                    varcount3++;
                    ...
                    for (l = 0; l < NLEVELS; l++) {
                        ...
                        count4d[3] = NCOLS;
                        status =
                            nc_put_vara_float(ncid, var_atts[v].id, start4d,
                                              count4d,
                                              temp_array);
                        Handle_Error(status);
                    }
...
// *************************************************************************************************
void
Close_VIC_Files(FILE **infilehandle)
{
    int k;
          ...
          struct tm date2)
{
    ...
    dsec =
        ((date1.tm_hour -
          date2.tm_hour) * 60 +
         (date1.tm_min - date2.tm_min)) * 60 + date1.tm_sec - date2.tm_sec;
...
// *************************************************************************************************
void
add_sec_date(struct tm  start_date,
             long long  sec_to_add,
             struct tm *new_date)
{
    int  nhour, nmin, nsec;
    long ndays;
    ...
    while (ndays) {
        ...
        if (ndays > 0 && ndays >= days_until_year_rollover) {
            ...
            while (ndays) {
                ...
                if (LEAPYR(new_date->tm_year + 1900) && new_date->tm_mon ==
                    1) {
                    // February of a leap year
                    days_in_month++;
                }
                if (ndays > 0) {
                    days_until_month_rollover = days_in_month -
                                                new_date->tm_mday + 1;
                }
                else if (ndays < 0) {
