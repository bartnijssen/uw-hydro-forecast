...
#$ -S /usr/bin/perl
#
# wrap_vic2nc.pl: Script to run the conversion tool vic2nc within SIMMA framework
#
...
$PARAMS_TEMPLATE = "$PARAMS_DIR/metadata.forcing.template";
...
$FORCING_DIR = "$PROJECT_DIR/forcing/$forcing_subdir/asc_disagg";
$RESULTS_DIR = "$PROJECT_DIR/forcing/$forcing_subdir/nc";
...
if ($local_storage) {
  $LOCAL_RESULTS_DIR = "$LOCAL_PROJECT_DIR/results/$forcing_subdir/nc";
...
$LOGFILE = "$LOGS_DIR/log.wrap_vic2nc.pl.$JOB_ID";
...
# Run the model
#$cmd = "$MODEL_EXE_DIR/$MODEL_EXE_NAME -i $FORCING_DIR -p $prefix -m $controlfile -o $results_dir/$prefix -c -t m >& $LOGFILE";
$cmd = "$MODEL_EXE_DIR/$MODEL_EXE_NAME -i $FORCING_DIR -p $prefix -m $controlfile -o $results_dir/$prefix -t m >& $LOGFILE";
...
sub usage() {
  ...
  print "\n";
  if ($_[0] eq "full") {
    print "Given vic-style ascii forcings, runs vic2nc (vic compiled with\n";
